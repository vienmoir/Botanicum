...
def leafCheck(imageName):
    ...
    if len(contours) == 0:
        result=False
        error = 'The leaf is too small. Please, take a closer shot!'
        return error,0,0
        ...
        if len(contours) == 0:
            ...
            if len(contours) > 1:
                    result=False
                    error = "This doesn't look like leaf at all!"
                    return error,0,0
            else:
                if len(contours) == 1:
                    cnt = contours[0]
                    ...
                    if x <= 1 or y <=1:
                        result=False
                        error = 'Error. The leaf exceeds the image borders'
                        return error,0,0
                        ...
                        result=False
                        error = 'Error. Please, adjust photo contrast or take another picture'
                        return error,0,0
                    elif area > max_size:
                        result=False
                        error = 'Please, change the lightning and take another picture'
                        return error,0,0
                        
    if result == True:
        print "The leaf has been processed successfully"
