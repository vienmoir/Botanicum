...
def leafCheck(img):
    ...
    if len(contours) == 0:
        ...
        if len(contours) == 0:
            result=False
            error = 'Ошибка. Сфотографируйте целый лист с черешком на однотонном фоне, пожалуйста.'
            return error
            ...
            if len(contours) > 1:
                    result=False
                    error = 'Ошибка. Сфотографируйте один лист на нейтральном фоне, пожалуйста.'
                    return error
            else:
                if len(contours) == 1:
                    cnt = contours[0]
                    ...
                    if x <= 1 or y <=1:
                        ...
                        result=False
                        error = 'Ошибка. Лист выходит за края изображения, сфотографируйте его полностью, пожалуйста.'
                        return error
                        ...
                        cv2.imshow('Image', img)
                        result=False
                        error = 'Ошибка. Сфотографируйте лист на контрастном фоне, пожалуйста.'
                        return error
    if result == True:
        # return img
        return 'ok'